...
class FreeBtns extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final freeProfilesFilteredByRegion = ref.watch(
      freeProfilesFilteredByRegionProvider,
    );
              ...
              child: GridView.builder(
                ...
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount:
                      MediaQuery.of(context).size.width <=
                              BottomSheetConst.maxWidth ||
                          freeProfiles.value!.length < 2
                      ? 1
                      : 2,
                  mainAxisSpacing: 8,
                  crossAxisSpacing: 8,
                ...
                itemBuilder: (context, index) {
                  ...
                  return FreeBtn(
                    freeProfile: profile,
                    onTap: () async {
                      final title = profile.title.en;
                      final consent = profile.consent.en;
                      final result = await ref
                          .read(dialogNotifierProvider.notifier)
                          .showFreeProfileConsent(
                            title: title,
                            consent: consent,
                          );
                      if (result == true) {
                        await ref
                            ...
                            .addManual(
                              ...
                              userOverride: UserOverride(
                                name: title,
                                updateInterval: 12,
                                enableWarp: profile.neededFeatures?.contains(
                                  'warp_over_proxies',
                                ),
                                enableFragment: profile.neededFeatures
                                    ?.contains('fragment'),
                              ),
                            );
                      }
          ...
          : Center(
              child: Text(
                (freeProfiles.value?.isEmpty ?? true)
                    ...
                    : t.pages.profiles.freeSubNotFoundForRegion(
